...
GLuint LoadSingleShader(const char * shaderFilePath, ShaderType type) 
{
	...
	//test InfoLogLength = 1 ?
	//printf("InfoLogLength = %d\n", InfoLogLength);


	if (InfoLogLength > 1) // Modify > -> < 9.28, Modify <0 -> >1
	{
...
GLuint LoadShaders(const char * vertexFilePath, const char * fragmentFilePath) 
{
	...
	//test, both IDs are 0 before modify "> 0"
	//printf("vertexShaderID = %d, fragmentShaderID = %d\n", vertexShaderID, fragmentShaderID);
	...
	if (InfoLogLength > 1) // Modify > -> < 9.28. Modify <0 -> >1 9/29
	{
